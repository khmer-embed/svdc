...
@else
    @if (isset($trash) && $trash)
        <button 
        id="btntrash_{{ $obj_info['name'] }}"
        type="button" 
        class="formactionbutton btn btn-outline-warning button-icon"><i class="fe fe-trash me-2"></i>
        @lang('dev.trash')
        @if(!empty($counttrash))
        <span class="badge  bg-warning rounded-pill ms-1">{{$counttrash}}</span>
        @endif
    </button>
    @endif
